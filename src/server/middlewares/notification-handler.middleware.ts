...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'content.create': {
    roles: ['owner', 'administrator', 'content-manager'],
    key: 'content-uploaded',
    tenantPath: [],
    userPath: [],
  },
  'watermark.update': {
    roles: ['owner', 'content-creator', 'content-manager'],
    key: 'watermark-updated',
    tenantPath: ['content_creator', 'user', 'administrator', 'watermark'],
    userPath: [],
  },
  'content.delete': {
    roles: ['owner', 'administrator', 'content-creator'],
    key: 'content-deleted',
    tenantPath: [],
    userPath: [],
  },
...
const tenantRoles: string[] = ['owner', 'administrator', 'content-creator', 'content-manager'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.content_creator.findFirst({
