...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['owner', 'administrator', 'content-creator', 'content-manager'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<ContentCreatorInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/content-creators` : null),
    () => getContentCreators({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
...

    {
      ...
      path: isTenantUser ? `/content-creators/view/${data?.[0]?.id}` : '/content-creators',
